...
function Install-LibreOffice {
    ...
    if (-Not $Remove) {
        ...
        $libreOfficeInstallGuid = (Get-ChildItem -Path HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall, HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall |
            Get-ItemProperty |
            Where-Object { $_.DisplayName -match "LibreOffice" }).PSChildName
        try {
            Start-Process "msiexec.exe" -ArgumentList "/x $libreOfficeInstallGuid /qn RebootYesNo=No" -Wait
        } catch {
            throw "LibreOffice removal failed when executing: $libreOfficeInstallGuid /x /qn RebootYesNo=No"
        }
